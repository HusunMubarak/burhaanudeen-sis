...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.auditLog.create({ data: { userId: session.user.id, action: "UPDATE", entity: "GalleryImage", entityId: id } });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.auditLog.create({ data: { userId: session.user.id, action: "DELETE", entity: "GalleryImage", entityId: id } });
